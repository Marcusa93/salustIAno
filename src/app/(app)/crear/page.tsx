  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-4 py-8 sm:px-6 sm:py-12">
      <header className="flex flex-col gap-2">
        <h1 className="font-display text-3xl text-foreground tracking-tight sm:text-4xl">Crear</h1>
        <p className="text-muted-foreground">Cuentos y canciones hechos para Salu.</p>
      </header>

      <div className="grid gap-4 sm:grid-cols-2">
        {OPTIONS.map((opt) => (
          <CreateCard key={opt.title} option={opt} />
        ))}
      </div>
    </div>
  );
